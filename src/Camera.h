 ...
 * ┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
 * ┃ file name: Camera.h                                                    ┃
 * ┃ create date: 2013年9月8日星期日 16时11分14秒                           ┃
...
public:
    ...
    QVector3D MovePos( void ) { return (m_Pos + m_Len); }
    ...
    void SetHorizontalAngle( float rotate);
    void SetRotateY( float rotateY, bool _try = false );
    void SetRotateH( float rotateH, bool _try = false );
    void ZoomIn( float step );
    float GetZoom(){ float scale = (m_Pos.z() + m_Len.z())*2.0f/m_Win_Height; return scale; }

    void SetPosFromWindows( int diff_x, int diff_y );
    QVector2D GetPosFromWindows( int diff_x, int diff_y );
    void ScreenToWindowsCoordinate( float &x, float &y);
    void WindowsCoordinateToWorldCoordinate( float &x, float &y);

    void Apply( void );
private:
    QVector3D           m_Pos;
    QVector3D           m_Len;
    float               m_RotateX, m_RotateY, m_RotateH;
